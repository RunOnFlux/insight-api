...
AddressController.prototype.utxoWithoutMempool = function(req, res) {
    ...
    this.node.getAddressUnspentOutputs(req.addrs, {queryMempool: false}, function(err, utxos) {
        ...
        async.map(utxos, self.transformUtxo.bind(self), function(err, results) {
          res.jsonp(results);
        })
    });
};
...
AddressController.prototype.multiutxo = function(req, res) {
  var self = this;
  this.node.getAddressUnspentOutputs(req.addrs, {}, function(err, utxos) {
    if(err && err.code === -5) {
      return res.jsonp([]);
    } else if(err) {
      return self.common.handleErrors(err, res);
...
AddressController.prototype.fluxnodemultiutxo = function(req, res) {
  var self = this;
  this.node.getAddressUnspentOutputs(req.addrs, {}, function(err, utxos) {
    if(err && err.code === -5) {
      return res.jsonp([]);
    ...
    async.map(utxos, self.transformUtxo.bind(self), function(err, results) {
      const fluxNodeUtxos = [];
      const curHeight = self.node.services.bitcoind.height;
      const networkName = self.node.network.name;
      const forkHeightAllowCumulus = networkName === 'testnet' ? 18000 : 1076532;
      const forkHeightDisallowCumulus = networkName === 'testnet' ? 24000 : 1086612;
      const forkHeightAllowNimbus = networkName === 'testnet' ? 22000 : 1081572;
      const forkHeightDisallowNimbus = networkName === 'testnet' ? 28000 : 1092372;
      const forkHeightAllowStratus = networkName === 'testnet' ? 26000 : 1087332;
      const forkHeightDisallowStratus = networkName === 'testnet' ? 32000 : 1097412;
      results.forEach((utxo) => {
        if (curHeight >= forkHeightAllowCumulus && utxo.satoshis === 1000 * 1e8)  { // mainnet height CUMULUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight >= forkHeightAllowNimbus && utxo.satoshis === 12500 * 1e8)  { // mainnet height NIMBUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight >= forkHeightAllowStratus && utxo.satoshis === 40000 * 1e8)  { // mainnet height STRATUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= forkHeightDisallowCumulus && utxo.satoshis === 10000 * 1e8)  { // mainnet height CUMULUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= forkHeightDisallowNimbus && utxo.satoshis === 25000 * 1e8)  { // mainnet height NIMBUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= forkHeightDisallowStratus && utxo.satoshis === 100000 * 1e8)  { // mainnet height STRATUS
          fluxNodeUtxos.push(utxo);
        }
      })
      res.jsonp(fluxNodeUtxos);
    })
  });
};

AddressController.prototype.transformUtxo = function(utxoArg, callback) {
  var utxo = {
  ...
  if (utxo.confirmations < 100) {
    // we need to know if it is coinbase or not
    this.txController.getTransaction(utxoArg.txid, function(err, transformedTx) {
      // always returns transformedTx or {}
      utxo.coinbase = transformedTx.isCoinBase;
      callback(null, utxo);
    })
  } else {
    callback(null, utxo);
