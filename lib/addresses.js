...
AddressController.prototype.fluxnodemultiutxo = function(req, res) {
  ...
  this.node.getAddressUnspentOutputs(req.addrs, {}, function(err, utxos) {
    ...
    async.map(utxos, self.transformUtxo.bind(self), function(err, results) {
      const fluxNodeUtxos = [];
      const curHeight = this.node.services.bitcoind.height;
      results.forEach((utxo) => {
        if (curHeight > 9999999 && utxo.satoshis === 1000 * 1e8)  {// mainnet height CUMULUS TODO
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight > 9999999 && utxo.satoshis === 12500 * 1e8)  {// mainnet height NIMBUS TODO
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight > 9999999 && utxo.satoshis === 40000 * 1e8)  {// mainnet height STRATUS TODO
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= 9999999 && utxo.satoshis === 10000 * 1e8)  {// mainnet height CUMULUS TODO
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= 9999999 && utxo.satoshis === 25000 * 1e8)  {// mainnet height NIMBUS TODO
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= 9999999 && utxo.satoshis === 100000 * 1e8)  {// mainnet height STRATUS TODO
          fluxNodeUtxos.push(utxo);
        }
      })
