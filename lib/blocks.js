...
BlockController.prototype.transformBlock = function (block, info, reward, transaction) {
  ...
  if (transaction && transaction.outputs) {
    transaction.outputs.forEach(function (output) {
      if (output.satoshis > (reward * 0.4) && output.address) {
  ...
  // Add PON vs POW specific fields
  if (blockObj.header.version >= 100) {
    // PON block
    result.isPON = true;
    result.blockType = 'Proof of Node';
    if (blockObj.header.nodesCollateral) {
      result.nodesCollateral = {
        hash: blockObj.header.nodesCollateral.hash ? blockObj.header.nodesCollateral.hash.toString('hex') : null,
        index: blockObj.header.nodesCollateral.index
      };
    }
    if (blockObj.header.vchBlockSig) {
      result.blockSignature = blockObj.header.vchBlockSig.toString('hex');
    }
  } else {
    // POW block
    result.isPON = false;
    result.blockType = 'Proof of Work';
    result.nonce = blockObj.header.nonce;
    result.solution = blockObj.header.solution;
  }
...
BlockController.prototype._getBlockSummary = function (hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    return async.waterfall([function (callback) {
      ...
      if (transaction && transaction.outputs) {
        transaction.outputs.forEach(function (output) {
          if (output.satoshis > (sreward * 0.4) && output.address) {
