  ...
  this.node.getAddressUnspentOutputs(req.addrs, {}, function(err, utxos) {
    ...
    async.map(utxos, self.transformUtxo.bind(self), function(err, results) {
      ...
      results.forEach((utxo) => {
        if (curHeight > 1076532 && utxo.satoshis === 1000 * 1e8)  { // mainnet height CUMULUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight > 1081572 && utxo.satoshis === 12500 * 1e8)  { // mainnet height NIMBUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight > 1087332 && utxo.satoshis === 40000 * 1e8)  { // mainnet height STRATUS
          fluxNodeUtxos.push(utxo);
        }
