...
AddressController.prototype.fluxnodemultiutxo = function(req, res) {
  ...
  this.node.getAddressUnspentOutputs(req.addrs, {}, function(err, utxos) {
    ...
    async.map(utxos, self.transformUtxo.bind(self), function(err, results) {
      ...
      const forkHeightAllowCumulus = self.node.network === 'testnet' ? 18000 : 1076532;
      const forkHeightDisallowCumulus = self.node.network === 'testnet' ? 24000 : 1086612;
      const forkHeightAllowNimbus = self.node.network === 'testnet' ? 22000 : 1081572;
      const forkHeightDisallowNimbus = self.node.network === 'testnet' ? 28000 : 1092372;
      const forkHeightAllowStratus = self.node.network === 'testnet' ? 26000 : 1087332;
      const forkHeightDisallowStratus = self.node.network === 'testnet' ? 32000 : 1097412;
