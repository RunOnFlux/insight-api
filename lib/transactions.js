...
TxController.prototype.getTransaction = function (txid, callback) {
  var self = this;
...
if (transaction.version != 5 && transaction.version != 6) {
  ...
  var FLUXNODE_TX_FEATURE_DELEGATES_BIT = 0x0100;  // Bit 8
  ...
  if (transaction.nType == FLUXNODE_START_TX_TYPE) {
    if (transaction.version == 5) {
      ...
      transformed.nFluxNodeTxVersion = transaction.nFluxNodeTxVersion;

      // Check for bit-based features
      var isNormalTx = (transaction.nFluxNodeTxVersion == FLUXNODE_NORMAL_TX_VERSION) ||
                       ((transaction.nFluxNodeTxVersion & 0x01) != 0);
      var isP2SHTx = (transaction.nFluxNodeTxVersion == FLUXNODE_P2SH_TX_VERSION) ||
                     ((transaction.nFluxNodeTxVersion & 0x02) != 0);

      if (isNormalTx && !(transaction.nFluxNodeTxVersion & 0x02)) {
        transformed.collateralPubKey = transaction.collateralPubKey;
      } else if (isP2SHTx) {
        transformed.redeemScript = transaction.redeemScript;
      }

      // Check for delegate feature
      if (transaction.nFluxNodeTxVersion & FLUXNODE_TX_FEATURE_DELEGATES_BIT) {
        transformed.hasDelegates = true;
        if (transaction.usingDelegates) {
          transformed.usingDelegates = transaction.usingDelegates;
          transformed.delegateData = transaction.delegateData;
        }
      }
    }
    transformed.zelnodePubKey = transaction.zelnodePubKey;
    transformed.fluxnodePubKey = transaction.zelnodePubKey;
  }
  if (transaction.nType == FLUXNODE_CONFIRM_TX_TYPE) {
...
TxController.prototype.transformFluxChainTransaction = function (transaction) {
