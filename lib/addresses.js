  ...
  this.node.getAddressUnspentOutputs(req.addrs, {}, function(err, utxos) {
    ...
    async.map(utxos, self.transformUtxo.bind(self), function(err, results) {
      ...
      results.forEach((utxo) => {
        if (curHeight > 1050000 && utxo.satoshis === 1000 * 1e8)  { // mainnet height CUMULUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight > 1050000 && utxo.satoshis === 12500 * 1e8)  { // mainnet height NIMBUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight > 1050000 && utxo.satoshis === 40000 * 1e8)  { // mainnet height STRATUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= 1086612 && utxo.satoshis === 10000 * 1e8)  { // mainnet height CUMULUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= 1092372 && utxo.satoshis === 25000 * 1e8)  { // mainnet height NIMBUS
          fluxNodeUtxos.push(utxo);
        }
        if (curHeight <= 1097412 && utxo.satoshis === 100000 * 1e8)  { // mainnet height STRATUS
          fluxNodeUtxos.push(utxo);
        }
      })
